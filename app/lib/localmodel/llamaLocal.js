...
export async function loadWasm() {
  ...
  if (typeof window !== 'undefined' && window.LlamaModule) {
    ...
    throw new Error("Llama WASM module (expected as window.LlamaModule) not loaded. Ensure llama.js is included via a script tag on the page.");
  }
}
...
export async function runModel(prompt) {
  if (!modelInstance) throw new Error("النموذج غير محمل (Model not loaded)");
